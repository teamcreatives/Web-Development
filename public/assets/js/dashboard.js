(function($) {
  ...
  $(function() {
    ...
    if ($("#traffic-chart").length) {
      ...
      var gradientStrokeYellow = ctx.createLinearGradient(0, 0, 0, 181);
      gradientStrokeYellow.addColorStop(0, 'rgba(255, 255, 0, 1)');
      gradientStrokeYellow.addColorStop(1, 'rgba(223, 255, 0, 1)');
      var gradientLegendYellow = 'linear-gradient(to right, rgba(255, 255, 0, 1), rgba(223, 255, 0, 1))';

      var gradientStrokeOrange = ctx.createLinearGradient(0, 0, 0, 181);
      gradientStrokeOrange.addColorStop(0, 'rgba(255, 124, 0, 1)');
      gradientStrokeOrange.addColorStop(1, 'rgba(255, 255, 255, 1)');
      var gradientLegendOrange = 'linear-gradient(to right, rgba(255, 124, 0, 1), rgba(255, 255, 255, 1))';
    ...
    if ($("#traffic").length) {
      ...
      var trafficOptions = {
        ...
        legendCallback: function(chart) {
          var text = []; 
          text.push('<ul>'); 
          for (var i = 0; i < trafficData.datasets[0].data.length; i++) { 
              text.push('<li><span class="legend-dots" style="background:' + 
              trafficData.datasets[0].legendColor[i] + 
                          '"></span>'); 
              if (trafficData.labels[i]) { 
                  text.push(trafficData.labels[i]); 
              }
              text.push('<span class="float-right">'+trafficData.datasets[0].data[i]+"%"+'</span>')
              text.push('</li>'); 
          } 
          text.push('</ul>'); 
          return text.join('');
      ...
      var trafficCanvas = $("#traffic").get(0).getContext("2d");
      var traffic = new Chart(trafficCanvas, {
        type: 'doughnut',
        data: trafficData,
        options: trafficOptions
      });
      $("#traffic-legend").html(traffic.generateLegend());  
    }


    if ($("#light-chart").length) {
      var gradientStrokeBlue = ctx.createLinearGradient(0, 0, 0, 181);
      gradientStrokeBlue.addColorStop(0, 'rgba(54, 215, 232, 1)');
      gradientStrokeBlue.addColorStop(1, 'rgba(177, 148, 250, 1)');
      var gradientLegendBlue = 'linear-gradient(to right, rgba(54, 215, 232, 1), rgba(177, 148, 250, 1))';
      ...
      var lightChartOptions = {
        ...
        legendCallback: function(chart) {
          var text = []; 
          text.push('<ul>'); 
          for (var i = 0; i < lightChartData.datasets[0].data.length; i++) { 
              text.push('<li><span class="legend-dots" style="background:' + 
              lightChartData.datasets[0].legendColor[i] + 
                          '"></span>'); 
              if (lightChartData.labels[i]) { 
                  text.push(lightChartData.labels[i]); 
              }
              text.push('<span class="float-right"> '+lightChartData.datasets[0].data[i]+"%"+'</span>')
              text.push('</li>'); 
          } 
          text.push('</ul>'); 
          return text.join('');
      ...
      var lightChartCanvas = $("#light-chart").get(0).getContext("2d");
      var lightChart = new Chart(lightChartCanvas, {
        type: 'doughnut',
        data: lightChartData,
        options: lightChartOptions
      });
      $("#light-chart-legend").html(lightChart.generateLegend());  
    }
